---
name: biz-deck
description: Monta decks de negócio prontos — pitch deck, deck de vendas, proposta comercial, QBR/relatório executivo. Fornece a sequência de slides e o ângulo de cada um; delega a geração/montagem ao codex-deck. Use quando o usuário quer um desses formatos específicos de negócio.
---

# codex-biz:biz-deck — decks de negócio

Especialização de negócio sobre o `codex-deck`. Aqui mora o **quê vai em cada slide** dos formatos clássicos; a geração de imagem e a montagem do `.pptx` são do `codex-deck`/`codex-image`.

## Requisito
`codex-deck` + `codex-image` instalados. Sem eles, entregue o outline dos slides pro usuário montar noutro lugar.
...
3. Passe o `outline.md` resultante pro `codex-deck:deck` (estágio-a-estágio: outline → estilo → amostra → gera → `.pptx`).
4. Estilo padrão de negócio: `limpo-profissional` ou `mckinsey` (ver `references/estilos.md` do codex-deck); título de ação no topo em QBR/proposta.
