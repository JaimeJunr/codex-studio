---
name: cartoon
description: Especialista em arte cartoon/toon pra jogos — sprites, personagens e assets com cores chapadas, contorno marcado (bold outline), formas exageradas e leitura limpa (estilo vetor/mobile game). Especialização de estilo sobre keyless-sprite: herda o fluxo game dev e injeta o bloco de estilo cartoon; pós = remoção de fundo pra alpha (sem pixelate). Depende de keyless-sprite e keyless-image.
---

# keyless-cartoon:cartoon — arte cartoon/toon

Especialização de **estilo** sobre `keyless-sprite`. Herda o fluxo `keyless-sprite:sprite` e troca só o bloco de estilo + o pós. Alvo: cartoon vetorial/mobile — cores chapadas, contorno grosso, apelo "cute/hero".
...
1. **Briefing / âncora / derivar** — igual à base pai, com o bloco de estilo cartoon e fundo chroma sólido. Consistência via edição referenciada (`keyless-image:edit`).
