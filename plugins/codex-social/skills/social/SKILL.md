---
name: social
description: Gera criativos on-brand pra redes sociais via Codex — capa/slides de carrossel, capa de story/reel, quote card, criativo de anúncio. Use quando o usuário quer a IMAGEM do post pronta (não só o texto). Depende do codex-image.
---

# codex-social:social — criativos on-brand pra redes

Gera a **imagem** do post via `codex-image` (Codex, keyless), on-brand. Não escreve a estratégia (isso é do fluxo de conteúdo do projeto) — aqui é a arte.
...
3. **Gerar via `codex-image:generate`** no aspect ratio certo, salvando no projeto (generate-then-move) — ex: `marketing/imagens/` ou `assets/social/`.
