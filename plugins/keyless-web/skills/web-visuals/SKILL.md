---
name: web-visuals
description: Gera visuais pra produção de site via Codex — hero image, OG/social card, ilustração de seção, background/textura e favicon. Use ao construir/refatorar um site e precisar de imagens on-brand coerentes entre si. Depende do keyless-image.
---

# keyless-web:web-visuals — visuais pra site

Gera os assets visuais de um site via `keyless-image` (Codex, keyless), mantendo **coerência visual entre os slots** (hero, cards, seções).

## Requisito
`keyless-image` instalado. Ideal: um `DESIGN.md`/guia de marca no projeto (paleta, tipografia, estilo de ilustração) pra tudo sair coerente.
...
3. **Gerar via `keyless-image:generate`**, salvando direto na árvore do site (`public/`, `assets/img/`) — generate-then-move.
