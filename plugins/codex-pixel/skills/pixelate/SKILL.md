---
name: pixelate
description: Especialista em pixel art autêntico pra jogos — sprites, sheets, tilesets e ícones com grid inteiro real, paleta limitada (DB16/PICO-8/NES), hard edges e sem anti-aliasing. Especialização de estilo sobre codex-sprite: herda o fluxo game dev e adiciona o bloco de estilo pixel + o pós obrigatório (downscale nearest + quantize de paleta). Depende de codex-sprite (fluxo + script) e codex-image.
---

# codex-pixel:pixelate — pixel art autêntico

Especialização de **estilo** sobre `codex-sprite`. Não reescreve o fluxo: **herda** `codex-sprite:sprite` (briefing → âncora → derivar → sheet) e injeta (a) o bloco de estilo pixel art e (b) o pós **obrigatório** que transforma a saída borrada da IA em pixel art de verdade. Fundamentos em `references/pixel-art.md`.

> **Herda as Regras de fluxo invioláveis** de `codex-sprite:sprite`: entreviste ANTES de gerar; você dita a ordem de produção (não pergunte "em que ordem"); contorne moderação/IP antes (design original desde o 1º prompt); sprite pequeno pede prompt sprite-oriented (chibi/overhead/formas simples).
...
Siga `codex-sprite:sprite`, com estas diferenças:
...
3. **Pós obrigatório** (script `spritekit.py` do codex-sprite, dependência):
   ```
   ...
   - **Paletas** (`--palette`): `db16`, `db32`, `pico8`, `nes`, `snes`, `gameboy`, `gameboy-gray`, `c64`, `cga`, `sweetie16`, `gray4/8/16`. Ver `references/pixel-art.md`; listar com `/codex-sprite:palette`.
   - **Backend Aseprite** (recomendado): se o usuário tem Aseprite, rode `/codex-sprite:setup` uma vez — o `pixelate` passa a usar o Aseprite pra quantize+dither de qualidade (imprime `[via aseprite]`), com `--dither ordered` fazendo dithering Bayer real. Sem Aseprite, cai no Pillow automaticamente (`[via Pillow]`, sem dither real).
4. **Sheet / GIF** — `spritekit.py sheet ... --json [--engine unity|godot]` e/ou `gif ... --fps 8` pra prévia do ciclo (ou `/codex-sprite:export`).
5. **Cleanup opcional** — clusters/órfãos/outline no Aseprite/LibreSprite.

## Atalhos

`/codex-pixel:new [grid] [paleta]` (ex.: `32 gameboy`) inicia o fluxo; `/codex-sprite:setup`, `/codex-sprite:palette`, `/codex-sprite:export` operam o script.
