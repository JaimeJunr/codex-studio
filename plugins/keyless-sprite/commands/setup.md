...
Configure o backend **Aseprite** do keyless-sprite. O `spritekit.py` usa o Aseprite quando disponível (quantize indexed + dithering + paleta `.gpl` de qualidade) e cai pro Pillow quando não há.
...
O caminho fica salvo em `~/.config/keyless-sprite/config.json`. Reporte a versão e o caminho resolvido. Se `--detect` falhar, peça o caminho do binário e rode `setup --path`.
